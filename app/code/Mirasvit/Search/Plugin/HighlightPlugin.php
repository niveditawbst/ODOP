...
class HighlightPlugin
{
    ...
    public function highlight($html, $query)
    {
        ...
        usort($preparedQuery, function ($a,$b){ return strlen($a) - strlen($b);});
        ...
        foreach ($matches[0] as $key => $match) {
            ...
            $strippedMatchWithHolder = str_ireplace(array_keys($queryWithHolders), $queryWithHolders, $strippedMatch);
            $replacement = str_ireplace($strippedMatch, $strippedMatchWithHolder, $match);
            if ($match != $replacement) {
                $html = str_ireplace($match, $replacement, $html);
            }
        }
