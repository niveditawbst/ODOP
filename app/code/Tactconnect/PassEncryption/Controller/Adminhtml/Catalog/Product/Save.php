...
namespace Tactconnect\PassEncryption\Controller\Adminhtml\Catalog\Product;
...
class Save extends \Magento\Catalog\Controller\Adminhtml\Product\Save
{
	...
	public function sanitizeRequest(){
		...
		$this->getRequest()->setParams($postValue);
	}
	...
	public function validateImageParam($postValues){
		...
		$postImage = isset($postValues['image']) ? $postValues['image'] : null;
		$postSmallImg = isset($postValues['small_image']) ? $postValues['small_image'] : null;
		$postThumbnail = isset($postValues['thumbnail']) ? $postValues['thumbnail'] : null;
		$postSwatchImg = isset($postValues['swatch_image']) ? $postValues['swatch_image'] : null;
		$postMediaGallery = isset($postValues['media_gallery']['images']) ? $postValues['media_gallery']['images'] : [];
		
		// main image
		if($postImage){
			$postImageExt = pathinfo($postImage, PATHINFO_EXTENSION);
			if(!in_array($postImageExt, $allowedImgType)){
				return false;
			}
			
			// check if file contain more than one extension
			$strCount = substr_count($postImage, '.'.$postImageExt);
			if($strCount > 1){
				return false;
			}
		}
		...
		// small image
		if($postSmallImg){
			$postSmallImgExt = pathinfo($postSmallImg, PATHINFO_EXTENSION);
			if(!in_array($postSmallImgExt, $allowedImgType)){
				return false;
			}
			
			// check if file contain more than one extension
			$strCount = substr_count($postSmallImg, '.'.$postSmallImgExt);
			if($strCount > 1){
				return false;
			}
		}
		...
		// thumbnail image
		if($postThumbnail){
			$postThumbnailExt = pathinfo($postThumbnail, PATHINFO_EXTENSION);
			if(!in_array($postThumbnailExt, $allowedImgType)){
				return false;
			}
			
			// check if file contain more than one extension
			$strCount = substr_count($postThumbnail, '.'.$postThumbnailExt);
			if($strCount > 1){
				return false;
			}
		}
		...
		// swatch image
		if($postSwatchImg){
			$postSwatchImgExt = pathinfo($postSwatchImg, PATHINFO_EXTENSION);
			if(!in_array($postSwatchImgExt, $allowedImgType)){
				return false;
			}
			
			// check if file contain more than one extension
			$strCount = substr_count($postSwatchImg, '.'.$postSwatchImgExt);
			if($strCount > 1){
				return false;
			}
		}
		
		if($postMediaGallery && is_array($postMediaGallery) && count($postMediaGallery)){
			foreach($postMediaGallery as $key => $postImage){
				$file = isset($postImage['file']) ? $postImage['file'] : null;
    ...
    public function execute()
    {
        ...
        $isValidImage = $this->validateImageParam($data);
        if(!$isValidImage){
			$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
			$redirectInterface = $objectManager->create('\Magento\Framework\App\Response\RedirectInterface');
			$refererUrl = $redirectInterface->getRefererUrl();
			
			$this->messageManager->addErrorMessage(__('Invalid Image !!!'));
			return $resultRedirect->setPath($refererUrl);
		}
        
        //~ $salt = 'd43e35e0916495462d675ad7463ba318';
        //~ $key = pack("H*", $salt.$salt);
		//~ $iv = pack("H*", $salt);
		//~ $decryptedPassword = base64_decode($this->getRequest()->getParam('hash_key'));
		//~ $decrypted = openssl_decrypt($decryptedPassword , 'aes-256-cbc', $key, OPENSSL_RAW_DATA, $iv); 
		
		//~ echo '<pre>@@@';
		//~ $a = 'W29iamVjdCBPYmplY3Rd';
		//~ print_r($decrypted);
		//~ print_r(base64_decode("QVRVTA=="));
		//~ exit;
		//~ $password = $decrypted;
