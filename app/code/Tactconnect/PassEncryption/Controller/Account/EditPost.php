...
class EditPost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    protected function changeCustomerPassword($email)
    {
        ...
        if ($this->getRequest()->getParam('change_password')) {
            ...
            $salt = $this->session->getFrontendChangePasswordEncryptionKey();
            $key = pack("H*", $salt.$salt);
	        $iv = pack("H*", $salt);
	        $currPass = base64_decode($currPass);
   			$currPass = openssl_decrypt($currPass , 'aes-256-cbc', $key, OPENSSL_RAW_DATA, $iv);
   			$newPass = base64_decode($newPass);
   			$newPass = openssl_decrypt($newPass , 'aes-256-cbc', $key, OPENSSL_RAW_DATA, $iv);
   			$confPass = base64_decode($confPass);
   			$confPass = openssl_decrypt($confPass , 'aes-256-cbc', $key, OPENSSL_RAW_DATA, $iv);
   			//echo $currPass.'::'.$newPass.'::'.$confPass;exit;
        ...
        $this->session->unsFrontendChangePasswordEncryptionKey();
